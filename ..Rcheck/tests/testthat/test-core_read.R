...
create_lna_with_dummy_run <- function(path) {
  ...
  run_group <- scans_group$create_group("run-01")      # Create a dummy run
  # Create data group and values dataset for no-transform case
  data_group <- run_group$create_group("data")
  data_group$create_dataset("values", array(1, dim=c(1,1,1,1)), gzip_level = 0)
  neuroarchive:::close_h5_safely(h5)
}
...
test_that("core_read handles empty /transforms group", {
  ...
  handle <- core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE)
...
test_that("core_read closes file if invert_step errors", {
  ...
  expect_error(core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE), "mock error")
...
test_that("core_read lazy=TRUE keeps file open", {
  ...
  handle <- core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = TRUE)
...
test_that("core_read output_dtype stored and float16 check", {
  ...
  h <- core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float64", roi_mask = NULL, time_idx = NULL, lazy = FALSE)
  expect_equal(h$meta$output_dtype, "float64")
  err <- expect_error(
    core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float16", roi_mask = NULL, time_idx = NULL, lazy = FALSE),
...
test_that("core_read allows float16 when support present", {
  ...
  h <- core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float16", roi_mask = NULL, time_idx = NULL, lazy = FALSE)
...
test_that("core_read works with progress handlers", {
  ...
  output <- testthat::capture_output_lines(progressr::with_progress(core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE)))
  if (length(output) > 0) {
    cat("\nCaptured output in 'core_read works with progress handlers':\n")
    cat(paste("  -", output), sep = "\n")
    cat("\n")
  }
  expect_silent(progressr::with_progress(core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE)))
})
...
test_that("core_read validate=TRUE calls validate_lna", {
  ...
  core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = TRUE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE)
  expect_true(called)
})
...
test_that("core_read allow_plugins='none' errors on unknown transform", {
  tmp <- local_tempfile(fileext = ".h5")
  create_lna_with_dummy_run_and_descriptor(tmp)
  expect_error(core_read(tmp, run_id = NULL, allow_plugins = "none", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE),
...
test_that("core_read allow_plugins='prompt' falls back when non-interactive", {
  ...
  expect_warning(
    core_read(tmp, run_id = NULL, allow_plugins = "prompt", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE),
    "Missing invert_step"
  )
})
...
test_that("core_read stores subset parameters", {
  ...
  h <- core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = roi, time_idx = 1:2, lazy = FALSE)
...
test_that("core_read run_id globbing returns handles", {
  ...
  handles <- core_read(tmp, run_id = "run-*", allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE)
...
test_that("core_read run_id globbing lazy returns first", {
  ...
  expect_warning(
    handle <- core_read(tmp, run_id = "run-*", allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = TRUE),
...
test_that("core_read validate=TRUE checks dataset existence", {
  ...
  expect_error(core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = TRUE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE),
               regexp = "HDF5 path '/scans/run-01/missing' not found")

  expect_no_error(core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE))
})
...
test_that("core_read validate=TRUE checks required params", {
  ...
  expect_no_error(core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = TRUE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE))
})
...
test_that("core_read reports step provenance from failing method", {
  ...
  err_cr_prov <- expect_error(core_read(tmp, run_id = NULL, allow_plugins = "installed", validate = FALSE, output_dtype = "float32", roi_mask = NULL, time_idx = NULL, lazy = FALSE))
  ...
  # The location should be "invert_step:fail:step0" (assuming index 0 for the "00_fail.json" descriptor).
  # We need to escape [ and ] for grepl if not using fixed = TRUE.
  # Using fixed = FALSE (default) for regexp behavior with brackets.
  expect_true(grepl("invert_step:fail:step0", trimws(err_cr_prov$location), fixed = TRUE))
