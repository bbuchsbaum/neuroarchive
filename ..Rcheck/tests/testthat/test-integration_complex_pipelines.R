...
assign("forward_step.aggregate_runs", .forward_step.aggregate_runs, envir = .GlobalEnv)
assign("invert_step.aggregate_runs", .invert_step.aggregate_runs, envir = .GlobalEnv)
...
withr::defer_parent({
  remove(list = c("forward_step.aggregate_runs", "invert_step.aggregate_runs"), envir = .GlobalEnv)
})
...
test_that("complex pipeline roundtrip", {
  testthat::local_mocked_bindings(
    default_params = function(type) {
      if (type == "aggregate_runs") return(list())
      if (type == "sparsepca") return(list(k=10)) # Assuming it might need some defaults
  ...
  write_lna(list(`run-01` = run1_data, `run-02` = run2_data), file = tmp,
            transforms = c("aggregate_runs", "sparsepca", "delta", "temporal"),
            transform_params = list(sparsepca = list(n_components = 3),
...
test_that("lna_reader subset on complex pipeline", {
  testthat::local_mocked_bindings(
    default_params = function(type) {
      if (type == "aggregate_runs") return(list())
      if (type == "sparsepca") return(list(k=10))
      if (type == "delta") return(list())
      if (type == "temporal") return(list())
...
test_that("checksum validation on complex pipeline", {
  ...
  write_lna(list(`run-01` = run1_data, `run-02` = run2_data), file = tmp,
            transforms = c("aggregate_runs", "sparsepca"),
            checksum = "sha256")
