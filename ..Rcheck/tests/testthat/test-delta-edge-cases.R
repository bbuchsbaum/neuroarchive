...
test_that("delta handles axis dimension of length 1", {
  ...
  for (coding in c("none", "rle")) {
    ...
    # Read the dataset using h5_read to handle placeholders correctly
    root <- h5[["/"]]
    delta_data <- neuroarchive:::h5_read(root, "/scans/run-01/deltas/00_delta/delta_stream")
    
    if (coding == "rle") {
      # Empty RLE should be a 0x2 matrix
      expect_true(is.matrix(delta_data))
      expect_equal(nrow(delta_data), 0)
      expect_equal(ncol(delta_data), 2)
    } else {
      # Direct encoding produces a 0xN matrix
      expect_true(is.matrix(delta_data))
      expect_equal(nrow(delta_data), 0)
      expect_equal(ncol(delta_data), 5)
    }
    h5$close_all()
...
test_that("delta handles prod(dims[-axis]) == 1", {
  ...
  for (coding in c("none", "rle")) {
    tmp <- local_tempfile(fileext = ".h5")
    h <- check_roundtrip(tmp, arr, axis = 1, coding = coding)
    expect_equal(h$stash$input, arr)
  }
})
...
test_that("delta handles axis dimension of length 0", {
  for (coding in c("none", "rle")) {
    ...
    h <- check_roundtrip(tmp1, arr, axis = 1, coding = coding)
    expect_equal(h$stash$input, arr)
    ...
    h <- check_roundtrip(tmp2, arr, axis = 1, coding = coding) 
    expect_equal(h$stash$input, arr) 
  }
})
